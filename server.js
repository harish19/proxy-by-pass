...
const init = async () => {
    ...
    const setCookies = function(req,cookies){
    	for (var i = 0; i < cookies.length; i++) {
    		...
    		req.state(rawCookie[0],rawCookie[1],{ firstVisit: false });
    	}
    	return req
    }
	...
	try{
		...
		server.route({
		  ...
		  handler: async function(req,h){
		  		...
		  		console.log('reqHeaders.cookie',req.state);
		  		if(req.method==='get'){
		  			var res = await Wreck.get(url,{headers:{cookie:reqHeaders.cookie||""},'content-type': 'application/json'});
		  			setCookies(h,res.res.headers['set-cookie']);
		  		 finalRes =  h.response(res.payload).header('content-type','application/json').header('Access-Control-Allow-Origin','*');
